...
# This catalog owns package interfaces as well as realizations, so its own
# `packages/interfaces/<name>/repro.nim` modules are importable by path from
# the repository root. `reprobuild-scoop-packages` needs no equivalent line:
# it owns no interfaces at all.
switch("path", thisDir())
